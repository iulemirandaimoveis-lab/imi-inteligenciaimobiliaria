  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-5">
      <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
        <Calculator size={18} className="text-[#C49D5B]" /> Simulador de Crédito Imobiliário
      </h3>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input type="number" value={valorImovel} onChange={e => setValorImovel(Number(e.target.value))}
              className="w-full h-9 pl-8 pr-3 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#C49D5B]" />
          </div>
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { l: sistema === 'PRICE' ? 'Parcela Fixa' : '1ª Parcela', v: formatCurrency(primeiraParcela), c: 'text-[#C49D5B] font-bold text-base' },
      ...
      <Link href="/backoffice/credito/novo"
        className="flex items-center justify-center gap-2 h-10 w-full bg-[#C49D5B] text-white rounded-xl text-sm font-semibold hover:bg-[#b08a4a] transition-colors">
...
export default function CreditoPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { l: 'Portfólio Total', v: formatCurrency(totalPortfolio), icon: DollarSign, c: 'text-[#C49D5B] bg-amber-50' },
      ...
      <div className="flex border-b border-gray-200">
        {[{ v: 'simulador', l: '🧮 Simulador' }, { v: 'operacoes', l: '📁 Operações' }].map(tab => (
          <button key={tab.v} onClick={() => setActiveTab(tab.v as any)}
            className={`px-5 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.v ? 'border-[#C49D5B] text-[#C49D5B]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            {tab.l}
          </button>
        ))}
      </div>
