...
| 1.4 | Triagem de TODOs/débito → inventário real (3 TODOs + 8 `@deprecated` + 3 stubs; "662" era falso positivo) e remoção dos shims deprecated sem uso (`lib/supabase.ts`, `lib/send-notification.ts`, `lib/design-system/tokens.ts`) | `src/lib/**` | Baixo | **Concluído** |
| 1.6 | Unificar formatadores BRL — há ≥3 (`commission.formatBRL` com centavos, `imi-proposals/status.formatBRL`, `utils.formatCurrency`, `format.formatCurrency` sem centavos). **A saída difere entre eles** (centavos, null → "—"), então a consolidação muda valores exibidos e exige verificação visual por tela | `lib/format.ts` + consumidores | Médio (UI-visível) | Pendente (junto com 1.2/1.3; JSDoc do deprecated corrigido para evitar migração cega) |
