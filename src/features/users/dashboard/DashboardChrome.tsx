...
import { ChevronDown, LogOut, Building2, Bell, BarChart3, LayoutDashboard, Users, FileText, ShieldCheck } from 'lucide-react'
...
export function DashboardTopbar({ projectName }: { projectName: string }) {
  ...
  const showNav = showIntelligence || showTeam || showProposals || showAdmin
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {showNav && (
          <nav className="imi-section-nav" style={{ display: 'flex', alignItems: 'center', gap: 4, padding: 3, borderRadius: T.rSm, background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.glassBorder}` }}>
            ...
            {showAdmin && <NavLink href="/users/admin" active={pathname.startsWith('/users/admin')} icon={<ShieldCheck size={14} />} label="Usuários" />}
          </nav>
        )}
        <button
          type="button"
