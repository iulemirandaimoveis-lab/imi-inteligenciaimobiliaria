...
import { ChevronDown, LogOut, Building2, Bell, BarChart3, LayoutDashboard, Users, FileText, Coins } from 'lucide-react'
...
export function DashboardTopbar({ projectName }: { projectName: string }) {
  ...
  const showNav = showIntelligence || showTeam || showProposals || showCommissions
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {showNav && (
          <nav className="imi-section-nav" style={{ display: 'flex', alignItems: 'center', gap: 4, padding: 3, borderRadius: T.rSm, background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.glassBorder}` }}>
            ...
            {showCommissions && <NavLink href="/users/commissions" active={pathname.startsWith('/users/commissions')} icon={<Coins size={14} />} label="Comissões" />}
