      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          <textarea ref={inputRef} className="mi" value={input} onChange={e => setInput(e.target.value)}
            ...
            style={{ flex: 1, padding: '10px 14px', borderRadius: 4, background: T.elevated, border: `1px solid ${T.border}`, color: T.text, fontSize: 12.5, resize: 'none', fontFamily: 'inherit', transition: 'all 150ms' }}
          />
          <button onClick={() => send()} disabled={loading || !input.trim()}
            style={{ width: 44, height: 44, borderRadius: 4, border: 'none', cursor: loading || !input.trim() ? 'not-allowed' : 'pointer', background: loading || !input.trim() ? T.surface : T.gold, color: loading || !input.trim() ? T.textDim : T.navy, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 150ms' }}>
      ...
      <div className="msb" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {QUICK_QUESTIONS.map(({ q, Icon }, i) => (
          <button key={i} onClick={() => send(q)} disabled={loading} className="mq"
            style={{ textAlign: 'left', padding: '10px 12px', borderRadius: 4, background: T.elevated, border: `1px solid ${T.border}`, color: T.textSub, fontSize: 11, cursor: 'pointer', lineHeight: 1.45, transition: 'all 150ms', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 900 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {[
          ...
          <div key={s.label} style={{ background: T.elevated, border: `1px solid ${T.border}`, borderRadius: 4, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 4, background: T.goldBg, border: `1px solid ${T.borderHi}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      {pages.length > 0 && (
        <div style={{ position: 'relative' }}>
          ...
          <input className="mi" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por título, arquivo ou norma..."
            style={{ width: '100%', padding: '9px 12px 9px 34px', borderRadius: 4, boxSizing: 'border-box', background: T.elevated, border: `1px solid ${T.border}`, color: T.text, fontSize: 12, transition: 'all 150ms' }}
          />
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '48px 24px', color: T.textSub, border: `1px dashed ${T.border}`, borderRadius: 4 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: T.elevated, border: `1px solid ${T.border}`, borderRadius: 4, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 34, height: 34, borderRadius: 4, background: T.goldBg, border: `1px solid ${T.borderHi}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <button onClick={() => del(p.id)} disabled={deleting === p.id} className="md"
                  style={{ width: 32, height: 32, borderRadius: 4, border: `1px solid ${T.border}`, background: 'transparent', color: T.textDim, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 150ms' }}>
...
function TabProcessar() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 720 }}>
      {/* Info banner */}
      <div style={{ background: T.goldBg, border: `1px solid ${T.borderHi}`, borderRadius: 4, padding: '16px 20px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
      ...
      <div onClick={() => inputRef.current?.click()}
        ...
        className="mdrop"
        style={{ border: `2px dashed ${isDragOver ? T.gold : T.border}`, borderRadius: 4, padding: '36px 24px', textAlign: 'center', cursor: 'pointer', background: isDragOver ? T.goldBg : files.length ? T.goldBg : 'transparent', transition: 'all 200ms' }}>
