  ...
  return (
    <div className="mgrid" style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 20 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
        ...
        {/* Messages */}
        <div style={{ background: T.elevated, border: `1px solid ${T.border}`, borderRadius: 10, padding: 16, minHeight: 360, maxHeight: 460, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {messages.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 10, opacity: 0.6, padding: '40px 0' }}>
              <div style={{ width: 48, height: 48, borderRadius: 12, background: T.goldBg, border: `1px solid ${T.borderHi}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div key={i} className={i === messages.length - 1 ? 'ani' : ''}
              ...
              {m.role === 'user' ? (
                <div style={{ maxWidth: '80%', padding: '10px 14px', borderRadius: '12px 4px 12px 12px', background: T.gold, color: T.navy, fontSize: 12.5, lineHeight: 1.6 }}>
                  {m.content}
                </div>
              ) : (
                <div className="mb" style={{ maxWidth: '82%', padding: '10px 14px', borderRadius: '4px 12px 12px 12px', background: T.surface, color: T.text, fontSize: 12.5, lineHeight: 1.65, border: `1px solid ${T.border}`, wordBreak: 'break-word' }}
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          <textarea ref={inputRef} className="mi" value={input} onChange={e => setInput(e.target.value)}
            ...
            style={{ flex: 1, padding: '10px 14px', borderRadius: 8, background: T.elevated, border: `1px solid ${T.border}`, color: T.text, fontSize: 12.5, resize: 'none', fontFamily: 'inherit', transition: 'all 150ms' }}
          />
          <button onClick={() => send()} disabled={loading || !input.trim()}
            style={{ width: 44, height: 44, borderRadius: 8, border: 'none', cursor: loading || !input.trim() ? 'not-allowed' : 'pointer', background: loading || !input.trim() ? T.surface : T.gold, color: loading || !input.trim() ? T.textDim : T.navy, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 150ms' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 900 }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '48px 24px', color: T.textSub, border: `1px dashed ${T.border}`, borderRadius: 10 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: T.elevated, border: `1px solid ${T.border}`, borderRadius: 8, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 720 }}>
      ...
      <div onClick={() => inputRef.current?.click()}
        ...
        className="mdrop"
        style={{ border: `2px dashed ${isDragOver ? T.gold : T.border}`, borderRadius: 10, padding: '36px 24px', textAlign: 'center', cursor: 'pointer', background: isDragOver ? T.goldBg : files.length ? T.goldBg : 'transparent', transition: 'all 200ms' }}>
      ...
      {files.length > 0 && (
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <button onClick={processAll} disabled={processing}
            style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 22px', borderRadius: 6, border: 'none', cursor: processing ? 'wait' : 'pointer', background: processing ? T.surface : T.gold, color: processing ? T.textDim : T.navy, fontSize: 12, fontWeight: 700, transition: 'all 150ms' }}>
          ...
          {!processing && (
            <button onClick={() => { setFiles([]); setResults([]) }}
              style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '10px 16px', borderRadius: 6, border: `1px solid ${T.border}`, background: 'transparent', color: T.textSub, fontSize: 12, cursor: 'pointer' }}>
      ...
      {done > 0 && (
        <div style={{ background: T.successBg, border: '1px solid rgba(45,143,92,0.25)', borderRadius: 8, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
