  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Link href="/backoffice/imoveis/novo"
            className="flex items-center gap-2 h-9 px-4 bg-[#1A1A2E] text-white rounded-xl text-sm font-semibold shadow-sm hover:shadow-md hover:bg-[#16162A] transition-all">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { l: 'VGV Global (Est.)', v: formatCurrency(totalValor), icon: DollarSign, c: 'text-[#3B82F6] bg-[#1A1A2E]/10' },
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2">
          <div className="relative flex-1 w-full">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full h-10 pl-9 pr-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#3B82F6] focus:border-transparent transition-all" />
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <button onClick={() => setShowFilters(!showFilters)}
              className={`flex-1 sm:flex-none flex justify-center items-center gap-2 h-10 px-4 border rounded-xl text-sm font-medium transition-colors ${showFilters ? 'border-[#3B82F6] text-[#3B82F6] bg-[#1A1A2E]/5' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
        ...
        {showFilters && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-gray-100">
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" value={precoMin} onChange={e => setPrecoMin(e.target.value)} placeholder="0"
                  className="w-full h-9 pl-8 pr-3 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-[#3B82F6] focus:outline-none" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <select value={status} onChange={e => setStatus(e.target.value)} className="w-full h-9 px-3 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-[#3B82F6] focus:outline-none bg-white">
              ...
              <div key={im.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all group flex flex-col h-full">
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <p className="text-xs text-gray-500 flex items-center gap-1.5 mb-3">
                    <MapPin size={12} className="text-[#3B82F6]" /> <span className="truncate">{im.location.neighborhood}, {im.location.city}</span>
                  </p>
                  ...
                  <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <div className="flex gap-1.5">
                      <Link href={`/imoveis/${im.slug}`} target="_blank"
                        className="w-8 h-8 rounded-full bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-[#1A1A2E] hover:text-white transition-colors">
              ...
              <div key={im.id} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 hover:bg-gray-50 transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-medium text-[#3B82F6] bg-[#1A1A2E]/10 px-2 py-0.5 rounded-full truncate max-w-[150px]">{im.developer}</span>
                  </div>
