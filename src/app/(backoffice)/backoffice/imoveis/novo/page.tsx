  ...
  return (
    <div style={{ minHeight: '100vh', background: T.navy, color: T.text }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: T.surface, borderTop: `1px solid ${T.border}`, padding: '14px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 90 }}>
        ...
        {step < 4 ? (
          <button type="button" onClick={next} style={{ position: 'relative', overflow: 'hidden', background: 'var(--btn-primary-bg)', border: '1px solid transparent', borderRadius: 6, padding: '10px 24px', fontSize: 13, fontWeight: 700, color: 'var(--btn-primary-text)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-sans)', transition: 'opacity var(--dur-2) var(--ease)' }}>Continuar<ArrowRight size={14} /><span style={{ position: 'absolute', bottom: 0, left: '12%', right: '12%', height: 2, background: 'linear-gradient(90deg, transparent, #C8A44A, transparent)', opacity: 0.6, pointerEvents: 'none' }} /></button>
        ) : (
          <button type="button" onClick={handleSave} disabled={saving} style={{ position: 'relative', overflow: 'hidden', background: saving ? 'var(--bg-elevated)' : 'var(--btn-primary-bg)', border: saving ? `1px solid ${T.border}` : '1px solid transparent', borderRadius: 6, padding: '10px 28px', fontSize: 13, fontWeight: 700, color: saving ? 'var(--text-secondary)' : 'var(--btn-primary-text)', cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-sans)', transition: 'all var(--dur-2) var(--ease)', opacity: saving ? 0.8 : 1 }}>
