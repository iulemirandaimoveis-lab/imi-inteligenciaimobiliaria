    ...
    <div style={{ minHeight: '100vh', background: T.navy, color: T.text }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: T.surface, borderTop: `1px solid ${T.border}`, padding: '14px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 90 }}>
        ...
        {step < 4 ? (
          ...
          <button type="button" onClick={handleSave} disabled={saving} style={{ background: saving ? T.elevated : T.gold, border: saving ? `1px solid ${T.border}` : 'none', borderRadius: 6, padding: '10px 28px', fontSize: 13, fontWeight: 700, color: saving ? T.textSub : T.navy, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-sans)', boxShadow: saving ? 'none' : 'var(--shadow-gold)', transition: 'all var(--dur-2) var(--ease)', opacity: saving ? 0.8 : 1 }}>
    ...
    <div style={{ minHeight: '100vh', background: T.navy, color: T.text }}>
      ...
      <div style={{ position: 'fixed', bottom: 56, left: 0, right: 0, background: T.surface, borderTop: `1px solid ${T.border}`, padding: '12px 16px', display: 'flex', gap: 10, zIndex: 90 }}>
        ...
        {step < 4 ? (
          ...
          <button type="button" onClick={handleSave} disabled={saving} style={{ flex: 1, height: 48, borderRadius: 6, background: saving ? T.elevated : T.gold, border: saving ? `1px solid ${T.border}` : 'none', fontSize: 14, fontWeight: 700, color: saving ? T.textSub : T.navy, cursor: saving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'var(--font-sans)', opacity: saving ? 0.85 : 1 }}>
