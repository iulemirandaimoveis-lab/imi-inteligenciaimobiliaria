...
export function Eyebrow({ children }: { children: React.ReactNode }) {
  return (
    <span style={{ fontSize: '8.5px', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--accent-400)', fontFamily: 'var(--font-outfit, sans-serif)', fontWeight: 700 }}>{children}</span>
  )
}

export function TabBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{ padding: '8px 18px', borderRadius: 6, background: active ? 'rgba(200,164,74,.12)' : 'transparent', border: active ? '1px solid rgba(200,164,74,.35)' : '1px solid transparent', color: active ? 'var(--accent-400)' : 'var(--text-secondary, #9FAAB8)', fontSize: '11px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', fontFamily: 'var(--font-outfit, sans-serif)', cursor: 'pointer', transition: 'all 0.2s' }}>{children}</button>
  )
}

export function StatCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color: string }) {
  return (
    <div style={{ background: 'var(--bg-surface, #162040)', border: '1px solid rgba(200,164,74,.18)', borderRadius: 6, padding: '20px', flex: 1 }}>
