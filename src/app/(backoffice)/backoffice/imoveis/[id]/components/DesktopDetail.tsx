  ...
  return (
    <div className="imovel-detail-wrap" style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px 48px' }}>
        ...
        <div style={{ marginLeft: 'auto', position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          ...
          <button
            ...
            style={{
              ...
              background: 'rgba(200,164,74,.08)', border: '1px solid rgba(200,164,74,.20)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          {statusOpen && (
            <div style={{
              ...
              border: '1px solid rgba(200,164,74,.20)', borderRadius: 10, padding: '6px 0',
      ...
      <div className="imovel-hero" style={{ position: 'relative', borderRadius: 12, overflow: 'hidden', height: 'min(65vh, 460px)', background: T.surface }}>
        {images.length > 0 ? (
          <>
            ...
            {images.length > 1 && (
              <>
                <button onClick={() => setGalleryIdx(i => (i - 1 + images.length) % images.length)} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', width: 36, height: 36, borderRadius: '50%', background: 'rgba(11,25,40,0.75)', border: '1px solid rgba(200,164,74,.30)', color: '#EBE7E0', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ChevronLeft size={18} /></button>
                <button onClick={() => setGalleryIdx(i => (i + 1) % images.length)} style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', width: 36, height: 36, borderRadius: '50%', background: 'rgba(11,25,40,0.75)', border: '1px solid rgba(200,164,74,.30)', color: '#EBE7E0', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ChevronRight size={18} /></button>
                <div style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(11,25,40,0.75)', border: '1px solid rgba(200,164,74,.20)', borderRadius: 6, padding: '4px 10px', ...MONO, fontSize: 11, color: 'rgba(235,231,224,0.8)' }}>{galleryIdx + 1} / {images.length}</div>
              </>
            )}
          </>
        ) : (
      ...
      <div className="imovel-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: 24, marginTop: 24, alignItems: 'start' }}>
        ...
        <div>
          {/* Sticky tab bar */}
          <div style={{ position: 'sticky', top: 0, zIndex: 10, background: 'var(--bg-base, var(--navy, #0B1120))', borderBottom: '1px solid rgba(200,164,74,.14)', marginBottom: 20 }}>
          ...
          {activeTab === 'overview' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ ...CARD, padding: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 12 }}>
                  {[
                    ...
                    <div key={label} style={{ background: 'var(--bg-surface)', border: '1px solid rgba(200,164,74,.10)', borderRadius: 8, padding: '12px 14px' }}>
                ...
                <div style={{ ...CARD, padding: 24 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {(dev.features ?? []).map((item, i) => (
                      <span key={i} style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(200,164,74,.08)', border: '1px solid rgba(200,164,74,.18)', color: T.textMuted, fontSize: 12, fontFamily: 'var(--font-outfit, sans-serif)', fontWeight: 500 }}>{item}</span>
                    ))}
                  </div>
                </div>
              )}
              <div style={{ ...CARD, padding: 24 }}>
                ...
                <div style={{ background: 'var(--bg-surface)', borderRadius: 8, overflow: 'hidden', height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(200,164,74,.10)', flexDirection: 'column', gap: 8, color: T.textDim }}>
              ...
              <div style={{ ...CARD, padding: 24 }}>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid rgba(200,164,74,.14)' }}>
                    ...
                    <tbody>
                      <tr style={{ background: 'rgba(200,164,74,.06)', borderBottom: '1px solid rgba(200,164,74,.10)' }}>
                      ...
                      {comparables.map((comp, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid rgba(200,164,74,.06)' }}>
              ...
              <div style={{ ...CARD, padding: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
                  {[
                    ...
                    <div key={label} style={{ background: 'var(--bg-surface)', border: '1px solid rgba(200,164,74,.10)', borderRadius: 8, padding: '16px 18px' }}>
              ...
              <div style={{ ...CARD, padding: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 20 }}>
                  {[
                    ...
                    <div key={label}>
                      ...
                      <input type="number" value={value} onChange={e => onChange(Number(e.target.value))} min={0} max={label.includes('%') ? 100 : undefined} style={{ width: '100%', padding: '10px 12px', borderRadius: 6, background: 'var(--bg-surface)', border: '1px solid rgba(200,164,74,.20)', color: T.text, outline: 'none', ...MONO, fontSize: 14, boxSizing: 'border-box' }} />
                    </div>
                  ))}
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                  {[
                    ...
                    <div key={label} style={{ background: 'var(--bg-surface)', border: '1px solid rgba(200,164,74,.10)', borderRadius: 8, padding: '14px 16px', textAlign: 'center' }}>
        ...
        <div style={{ position: 'sticky', top: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ ...CARD, padding: 24 }}>
            ...
            <div style={{ height: '1px', background: 'rgba(200,164,74,.14)', margin: '16px 0' }} />
            <IMIScoreDisplay score={score} />
            {score > 0 && (
            ...
            <div style={{ ...CARD, padding: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {dev.developer.logo_url ? (
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: 6, flexShrink: 0, background: 'rgba(200,164,74,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
