...
export default function NovaConsultoriaPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Erro ao criar consultoria')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div key={s} className="flex items-center flex-1">
            ...
            {i < steps.length - 1 && (
              <div className="h-px flex-1 mx-3"
                style={{ background: step > i + 1 ? 'var(--success)' : T.border }} />
            )}
          </div>
        ))}
      </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={f.k}>
                  ...
                  <input value={(form as Record<string, any>)[f.k]} onChange={e => set(f.k, e.target.value)}
