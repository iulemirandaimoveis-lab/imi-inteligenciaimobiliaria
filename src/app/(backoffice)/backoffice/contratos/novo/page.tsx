...
const PLATAFORMAS = [
    ...
    { id: 'clicksign', label: 'ClickSign', sub: 'Juridicamente válido BR · WhatsApp integrado · ~R$99/mês', icon: FileSignature, cor: '#486581', gratuito: false, env: 'CLICKSIGN_ACCESS_TOKEN' },
            ...
            <div className="flex gap-2">
                {['pessoa_fisica', 'pessoa_juridica'].map(t => (
                    <button key={t} onClick={() => onChange({ ...parte, tipo: t })}
                        ...
                        style={{ background: parte.tipo === t ? '#486581' : T.elevated, color: parte.tipo === t ? 'white' : T.textDim, border: `1px solid ${parte.tipo === t ? T.borderGold : T.border}` }}>
                        ...
                        <div key={s.id} className="flex items-center flex-shrink-0">
                            <div className="flex flex-col items-center">
                                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
                                    style={{ background: i < step ? '#6BB87B' : i === step ? '#486581' : T.surface, color: i <= step ? 'white' : T.textDim, border: i > step ? `1px solid ${T.border}` : 'none' }}>
                                    {i < step ? '✓' : i + 1}
                                </div>
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-0.5">
                                {['todos', ...Object.keys(CATEGORIAS_LABEL)].map(cat => (
                                    <button key={cat} onClick={() => setCatF(cat)} className="px-3 h-8 rounded-xl text-xs font-semibold flex-shrink-0 transition-all"
                                        style={{ background: catF === cat ? '#486581' : T.elevated, color: catF === cat ? 'white' : T.textDim, border: `1px solid ${catF === cat ? T.borderGold : T.border}` }}>
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        {idiomasSel.map(l => (
                                            <button key={l} onClick={() => setIdiomaPrim(l)} className="flex items-center gap-1.5 px-3 h-8 rounded-xl text-xs font-semibold transition-all"
                                                style={{ background: idiomaPrim === l ? '#486581' : T.elevated, color: idiomaPrim === l ? 'white' : T.textDim, border: `1px solid ${idiomaPrim === l ? T.borderGold : T.border}` }}>
                        ...
                        <div className="space-y-2">
                            {idiomasSel.map(lang => (
                                <motion.button key={lang} whileTap={{ scale: 0.97 }} onClick={() => gerar(lang)} disabled={gerando}
                                    ...
                                    style={{ background: gerando ? 'rgba(26,26,46,0.18)' : '#486581', boxShadow: gerando ? 'none' : '0 1px 2px rgba(0,0,0,0.1)' }}>
                    ...
                    <motion.div key="result" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4">
                        ...
                        <div className="rounded-2xl p-5 space-y-4" style={{ background: T.surface, border: `1px solid ${T.border}` }}>
                            ...
                            <div className="flex gap-2">
                                {(['email', 'whatsapp', 'ambos'] as const).map(c => (
                                    <button key={c} onClick={() => setCanal(c)} className="flex-1 flex items-center justify-center gap-2 h-10 rounded-xl text-xs font-semibold transition-all"
                                        style={{ background: canal === c ? '#486581' : T.elevated, color: canal === c ? 'white' : T.textDim, border: `1px solid ${canal === c ? T.borderGold : T.border}` }}>
                            ...
                            {envioOk
                                ...
                                : <button onClick={enviar} disabled={enviando} className="w-full h-12 rounded-xl text-sm font-semibold text-white flex items-center justify-center gap-2" style={{ background: '#486581', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}>
                ...
                <div className="flex justify-between pt-2">
                    ...
                    {step < STEPS.length - 1 && (
                        <button onClick={() => setStep(s => s + 1)} disabled={!ok()} className="flex items-center gap-2 h-10 px-6 rounded-xl text-sm font-semibold text-white transition-all"
                            style={{ background: ok() ? '#486581' : 'rgba(26,26,46,0.15)', opacity: ok() ? 1 : 0.5, boxShadow: ok() ? '0 1px 2px rgba(0,0,0,0.1)' : 'none' }}>
...
export default function NovoContratoPage() {
    return (
        <Suspense fallback={<div className="flex h-64 items-center justify-center"><Loader2 size={24} className="animate-spin" style={{ color: '#486581' }} /></div>}>
            <NovoContratoInner />
        </Suspense>
    )
}
