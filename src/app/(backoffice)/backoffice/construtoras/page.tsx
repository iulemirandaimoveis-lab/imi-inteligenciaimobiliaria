...
export default async function BackofficeConstrutorasPage() {
  ...
  try {
    const { data: developersData, error } = await supabase
      .from('developers')
      .select('id, name, legal_name, cnpj, email, phone, address, is_active, logo_url, developments(id)')
    ...
    developers = (developersData || []).map((dev) => ({
      id: dev.id,
      name: dev.name,
      ...
      // Contagem de empreendimentos baseada na relação real do banco de dados
      empreendimentosAtivos: dev.developments?.length || 0,
    ...
    console.error('Erro inesperado ao buscar construtoras:', err)
  }
