...
const EVENT_TYPES: Record<string, { label: string; color: string; icon: any }> = {
    ...
    evento: { label: 'Evento', color: '#3B82F6', icon: Calendar },
                        ...
                        <div className={T.card}>
                            ...
                            <div className="grid grid-cols-7">
                                ...
                                {Array.from({ length: daysInMonth }).map((_, i) => {
                                    ...
                                    return (
                                        <div key={dayNum}
                                            className={`min-h-[80px] sm:min-h-[100px] border-b border-r border-white/[.04] p-1.5 sm:p-2 ${isToday ? 'bg-[#1A1A2E]/5' : ''}`}>
                                            <span className={`text-xs font-bold ${isToday ? 'text-[#3B82F6]' : 'text-white/60'}`}>
                                                {dayNum}
                                            </span>
