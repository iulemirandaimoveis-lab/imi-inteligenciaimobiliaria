...
function AgendaSkeleton() {
  return (
    <div className="space-y-5">
      {/* Week strip skeleton */}
      <div style={{ height: 120, background: 'var(--bg-surface)', borderRadius: 18, opacity: 0.5 }} />
      {/* Day events skeleton */}
      <div style={{ borderRadius: 18, overflow: 'hidden', background: 'var(--bg-surface)', opacity: 0.45 }}>
        <div style={{ height: 52, borderBottom: '1px solid var(--border-default)' }} />
        {[0, 1, 2].map(i => (
          <div key={i} className="flex items-center gap-4 px-5 py-4" style={{ borderBottom: i < 2 ? '1px solid var(--border-default)' : 'none' }}>
            <div style={{ width: 46, height: 40, borderRadius: 10, background: 'var(--bg-elevated)', opacity: 0.6 }} />
            <div style={{ width: 3, height: 40, borderRadius: 2, background: 'var(--border-default)' }} />
            <div style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--bg-elevated)', opacity: 0.5 }} />
            <div style={{ flex: 1 }}>
              <div style={{ height: 14, width: '60%', background: 'var(--bg-elevated)', borderRadius: 6, marginBottom: 6, opacity: 0.7 }} />
              <div style={{ height: 11, width: '35%', background: 'var(--bg-elevated)', borderRadius: 6, opacity: 0.4 }} />
            </div>
          </div>
        ))}
      </div>
    ...
    fetch('/api/leads?limit=30')
      ...
      .then(data => {
        ...
        if (hotLead) suggestions.push({
          ...
          action: 'Agendar Agora', icon: Zap, color: 'var(--error)',
        })
        ...
        if (qualified) suggestions.push({
          ...
          action: 'Criar Reunião', icon: Users, color: 'var(--success)',
        })
    ...
    <div className="space-y-5">
          ...
          actions={
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div style={{
                ...
                border: '1px solid var(--border-default)', borderRadius: '12px', padding: '3px',
              }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            <button
              ...
              style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
      ...
      {/* ── Day Events + Suggestions + Week Overview ────────────────────── */}
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <div style={{ background: 'var(--glass-surface)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid var(--glass-surface-border)', borderRadius: '10px', overflow: 'hidden', boxShadow: 'var(--glass-surface-shadow)' }}>
              {/* Header */}
              <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-default)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button
                  onClick={() => setShowModal(true)}
                  style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', cursor: 'pointer' }}
                >
              ...
              {selectedDayEvents.length === 0 ? (
                <div style={{ padding: '48px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', textAlign: 'center' }}>
                  <Calendar size={36} color="var(--text-secondary)" style={{ opacity: 0.3 }} />
                  <p style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>Nenhum evento neste dia</p>
                  <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Toque em + para adicionar</p>
                </div>
              ) : (
                <div>
                  {selectedDayEvents.map((ev, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        style={{
                          padding: '16px 20px',
                          borderBottom: idx < selectedDayEvents.length - 1 ? '1px solid var(--border-default)' : 'none',
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                            <h3 style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>
                              {ev.title}
                            </h3>
                          ...
                          {ev.location && (
                            <p style={{ fontSize: '11px', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              <MapPin size={11} />
                              {ev.location}
                            </p>
                          )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                          {isOnline ? (
                            ...
                            <button style={{ display: 'flex', alignItems: 'center', gap: '5px', height: '32px', padding: '0 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 700, color: 'var(--text-secondary)', background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', cursor: 'pointer' }}>
                              <Navigation size={13} />
                              Rota
                            </button>
                          )}
                          <button
                            ...
                            style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', background: 'transparent', border: 'none', cursor: 'pointer' }}
                          >
          ...
          {activeSuggestions.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}>
              <div style={{ background: 'var(--glass-surface)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid var(--glass-surface-border)', borderRadius: '10px', overflow: 'hidden', boxShadow: 'var(--glass-surface-shadow)', backgroundImage: 'linear-gradient(135deg, rgba(129,140,248,0.04) 0%, transparent 50%)' }}>
                {/* Header */}
                <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-default)', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)' }}>Sugestões da IA</span>
                  ...
                  {activeSuggestions.map((sug, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        style={{
                          padding: '14px 20px',
                          borderBottom: idx < activeSuggestions.length - 1 ? '1px solid var(--border-default)' : 'none',
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                          {dayEvts.slice(0, 3).map(ev => {
                            const cfg = getTypeCfg(ev.event_type)
                            return (
                              <div key={ev.id} style={{ borderRadius: '6px', padding: '2px 5px', fontSize: '9px', fontWeight: 600, color: cfg.color, background: cfg.bg, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {ev.title}
                              </div>
                            )
                          })}
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              style={{ width: '100%', maxWidth: '500px', background: 'var(--glass-surface)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid var(--glass-surface-border)', borderRadius: '10px', boxShadow: 'var(--glass-surface-shadow)', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div>
                  <label style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: '6px' }}>Título *</label>
                  <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Nome do evento" style={inputStyle} />
                </div>

                <div>
                  <label style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: '6px' }}>Tipo</label>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    <label style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: '6px' }}>Início *</label>
                    <input type="datetime-local" value={form.start_time} onChange={e => setForm({ ...form, start_time: e.target.value })} style={inputStyle} />
                  </div>
                  <div>
                    <label style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: '6px' }}>Fim</label>
                    <input type="datetime-local" value={form.end_time} onChange={e => setForm({ ...form, end_time: e.target.value })} style={inputStyle} />
                  </div>
                </div>

                <div>
                  <label style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: '6px' }}>Local / Link da Reunião</label>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                  ...
                  <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Dia inteiro</span>
                </label>
              </div>

              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setShowModal(false)} style={{ flex: 1, height: '44px', borderRadius: '6px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', background: 'var(--bg-surface)', color: 'var(--text-secondary)', border: '1px solid var(--border-default)' }}>
                  Cancelar
                </button>
                <button
