 ...
 * e-mail/cadastro. Cria uma sala Daily.co sob demanda (POST /api/video-call),
 * avisa o corretor por WhatsApp com o link, e abre a sala num iframe aqui
 * mesmo no site. Se o Daily.co não estiver configurado (DAILY_API_KEY ausente)
 * ou a chamada falhar, cai graciosamente para o WhatsApp — nunca trava o CTA.
 */
...
export default function VideoCallButton({ brokerName, brokerPhone, clientName, context, compact }: Props) {
  ...
  const whatsappFallback = `https://wa.me/${brokerPhone.replace(/\D/g, '')}?text=${encodeURIComponent(
  ...
  return (
    <>
      ...
      >
        ...
        {state === 'loading' ? 'Chamando…' : compact ? 'Vídeo chamada' : 'Vídeo chamada com o corretor'}
      </button>
            ...
            <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              <div className="flex items-center gap-2">
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: '#fff' }}>
                  Chamando {brokerName}… avisamos pelo WhatsApp
                </span>
              </div>
              <div className="flex items-center gap-2">
                <a
