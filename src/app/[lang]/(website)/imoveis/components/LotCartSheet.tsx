...
/**
 * LotCartSheet — resumo do carrinho de lotes (proposta em andamento).
 * Extraído de AltoBellevueGeoMap para ficar disponível nas 3 vistas do mapa
 * (Plano, Satélite + Lotes, Satélite), já que o carrinho é compartilhado via
 * localStorage (useLotCart) independentemente da vista ativa.
 *
 * Sempre `fixed` (não `absolute`) com z-index acima de qualquer barra fixa da
 * página (ex.: MobileStickyBar, z-[140]) para nunca ficar escondido atrás dela.
 */

import { useEffect } from 'react';
import { motion } from 'framer-motion';
import { ShoppingCart, Trash2, Check, Link2, FileText, X } from 'lucide-react';
...
const fmtBRL = (v: number) =>
  new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 }).format(Math.round(v || 0));
const fmtM2 = (v: number) => `${Math.round(v || 0).toLocaleString('pt-BR')} m²`;

export default function LotCartSheet({
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed z-[9999] left-0 right-0 bottom-0 sm:left-auto sm:right-4 sm:bottom-4 sm:w-[380px] rounded-t-[24px] sm:rounded-[20px] flex flex-col overflow-hidden"
        style={{
