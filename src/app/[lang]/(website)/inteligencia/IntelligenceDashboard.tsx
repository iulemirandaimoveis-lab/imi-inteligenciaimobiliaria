...
import {
  ...
  ChevronDown, ChevronUp, Percent, Clock, DollarSign,
...
const BRAZIL_CITIES = BRAZIL_FALLBACK_CITIES.map((c) => ({
  name: c.city,
  key: c.city,
  state: c.state,
}))
    ...
    countries: [
      {
        ...
        flag: '🇦🇪',
        cities: [{ name: 'Dubai', key: 'Dubai', state: '' }],
      },
    ],
  },
]
...
export default function IntelligenceDashboard({
  ...
  const initialCity =
    ALL_CITIES.find(
      (city) =>
        city.state.toLowerCase() === (initialState ?? '').toLowerCase() &&
        slugify(city.name) === initialCitySlug,
    )?.key ?? ALL_CITIES[0].key

  const [selectedLocation, setSelectedLocation] = useState({
    state: initialState ?? '',
    municipality: initialCity,
    neighborhood: '',
  })
    ...
    () =>
      selectedLocation.neighborhood
        ? neighborhoods.filter((n) =>
            n.neighborhood.toLowerCase().includes(selectedLocation.neighborhood.toLowerCase()),
          )
        : neighborhoods,
    [neighborhoods, selectedLocation.neighborhood],
  )

  const validPrices = displayedNeighborhoods.filter((n) => n.median_price_sqm != null)
  const cityAvgPrice =
    validPrices.length > 0
      ? Math.round(
          validPrices.reduce((s, n) => s + Number(n.median_price_sqm), 0) / validPrices.length,
        )
      : null
  ...
  const currentGeo = GEO.find((g) =>
    g.countries.some((c) => c.cities.some((ci) => ci.key === selectedCity)),
  )
  const currentCountry = currentGeo?.countries.find((c) =>
    c.cities.some((ci) => ci.key === selectedCity),
  )
  ...
  // Breadcrumb parts: Global › Continent › Country › State › City › [Neighborhood]
  const breadcrumbParts = [
    { label: 'Global', href: null },
    { label: currentGeo?.continent ?? 'América do Sul', href: null },
    { label: `${currentCountry?.flag ?? '🇧🇷'} ${currentCountry?.name ?? 'Brasil'}`, href: null },
    ...(currentMunicipality?.state
      ? [{ label: currentMunicipality.state, href: `/${lang}/inteligencia/brasil/${currentMunicipality.state.toLowerCase()}` }]
      : []),
    { label: currentCityObj?.name ?? selectedCity, href: null, highlight: true },
    ...(selectedLocation.neighborhood
      ? [{ label: selectedLocation.neighborhood, href: null, gold: true }]
      : []),
  ]
  ...
  return (
    <main className="bg-[#0B1928] min-h-screen">
      ...
      <section className="relative bg-[#0B1928] text-white pt-20 pb-12 md:pt-28 md:pb-18 overflow-hidden border-b border-white/[0.05]">
        <div
          className="absolute top-0 right-0 w-2/3 h-full opacity-[0.05]"
        ...
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(rgba(200,164,74,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(200,164,74,0.4) 1px, transparent 1px)',
        ...
        <div className="container-custom relative z-10">
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-[#C8A44A] font-bold uppercase tracking-[0.3em] text-[10px]">
              Market Intelligence
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <HeroBadge icon="📍" label="Todos os estados cobertos" />
            <HeroBadge icon="🏙️" label="Dados por bairro" />
            <HeroBadge icon="📈" label="Atualização diária" />
          </div>
        </div>
      </section>
          ...
          >
            {breadcrumbParts.map((part, i) => (
              <span key={i} className="flex items-center gap-1">
                {i > 0 && <ChevronRight className="w-2.5 h-2.5 opacity-30 shrink-0" />}
                <span
                  className={
            ...
            <div className="flex-1">
              <LocationSearchPanel
                ...
                onMunicipalitySelect={(municipalityName, stateUf) => {
                  ...
                  router.replace(
                    `/${lang}/inteligencia/brasil/${stateUf.toLowerCase()}/${slugify(municipalityName)}`,
                  )
                }}
        ...
        <div className="container-custom">
          <div className="grid grid-cols-3 gap-2 md:gap-3">
            <CompactMetric
              ...
              label="Preço médio/m²"
              value={dataState === 'loading' ? null : fmtCurrency(cityAvgPrice)}
            ...
            <CompactMetric
              ...
              value={
                dataState === 'loading' || !bestYield
                  ? null
                  : `${Number(bestYield.avg_rental_yield).toFixed(1)}% a.a.`
              }
              loading={dataState === 'loading'}
              highlight
            />
            <CompactMetric
              ...
              label={
                fastestSelling
                  ? `Vende mais rápido · ${fastestSelling.neighborhood}`
                  : 'Venda mais rápida'
              }
              value={
                dataState === 'loading' || !fastestSelling
                  ? null
                  : `${fastestSelling.avg_days_on_market}d`
              }
              loading={dataState === 'loading'}
            />
          </div>
      ...
      <section className="py-10 md:py-14">
        <div className="container-custom">
          ...
          <div className="flex items-end justify-between mb-8">
            <div>
              ...
              <h2 className="font-display text-xl sm:text-2xl font-bold text-white leading-tight">
                Panorama em{' '}
                <span className="text-[#C8A44A] italic">
          ...
          {dataState === 'loading' ? (
            ...
            <EmptyState city={currentCityObj?.name ?? selectedCity} lang={lang} />
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {displayedNeighborhoods.map((n, i) => (
                <NeighborhoodCard
                  ...
                  index={i}
                  rank={i + 1}
                  total={displayedNeighborhoods.length}
      ...
      <section className="bg-[#0B1928] text-white py-20 md:py-28 text-center relative overflow-hidden border-t border-white/[0.05]">
        <div
          ...
          style={{
            backgroundImage:
              'radial-gradient(ellipse at center, rgba(200,164,74,0.07) 0%, transparent 65%)',
          }}
        />
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(rgba(200,164,74,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(200,164,74,0.5) 1px, transparent 1px)',
        ...
        <div className="container-custom relative z-10 max-w-lg mx-auto">
          <div className="flex items-center justify-center gap-3 mb-5">
            ...
            <span className="text-[#C8A44A] font-bold uppercase tracking-[0.28em] text-[10px]">
              IMI Consultoria
            </span>
            <div className="w-8 h-px bg-[#C8A44A] opacity-40" />
          </div>
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div
          className={`text-sm md:text-base font-bold leading-none tracking-tight ${
            highlight ? 'text-[#C8A44A]' : 'text-white'
          }`}
        >
          {value ?? '--'}
        </div>
...
function NeighborhoodCard({
  ...
  const priceDiffLabel =
    cityAvgPrice != null && price != null
      ? price > cityAvgPrice
        ? `${((price / cityAvgPrice - 1) * 100).toFixed(0)}% acima da média`
        : `${((1 - price / cityAvgPrice) * 100).toFixed(0)}% abaixo da média`
      : null

  return (
    <div
      onClick={onToggle}
      className="group rounded-xl bg-[#0B1928] border border-white/[0.06] hover:border-white/[0.14] transition-all duration-300 cursor-pointer overflow-hidden relative"
      style={{
      ...
      <div className="p-4 pl-5">
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-1 shrink-0 ml-2" style={{ color: trendColor }}>
            ...
            <span className="text-xs font-bold">{fmtPct(trend)}</span>
          </div>
        </div>
          ...
          {[
            {
              label: 'Rentab.',
              value:
                data.avg_rental_yield != null
                  ? `${Number(data.avg_rental_yield).toFixed(1)}%`
                  : '--',
            },
            {
              label: 'Liquidez',
              value:
                data.absorption_rate != null
                  ? `${Number(data.absorption_rate).toFixed(1)}%`
                  : '--',
            },
            {
              label: 'Val. 12m',
              value:
                data.price_trend_12m != null ? fmtPct(Number(data.price_trend_12m)) : '--',
            },
          ].map(({ label, value }) => (
            ...
            {[
              {
                ...
                value:
                  data.inventory_count != null ? `${data.inventory_count} imóveis` : '--',
              },
              {
                label: 'Tempo médio de venda',
                value:
                  data.avg_days_on_market != null ? `${data.avg_days_on_market} dias` : '--',
              },
              {
                label: 'Absorção/mês',
                value:
              ...
              },
              {
                ...
                value:
                  data.vacancy_rate != null
                    ? `${Number(data.vacancy_rate).toFixed(1)}%`
                    : '--',
              },
              {
                label: 'Aluguel/m²',
                value:
                  data.avg_monthly_rent_sqm != null
                    ? fmtCurrency(Number(data.avg_monthly_rent_sqm))
                    : '--',
              },
              {
                label: 'Lançamentos 12m',
                value: data.new_launches_12m != null ? `${data.new_launches_12m}` : '--',
              },
              {
                label: 'Valorização 5a',
                value:
                  data.valorization_5y != null
                    ? fmtPct(Number(data.valorization_5y))
                    : '--',
              },
              {
                label: 'Transporte',
                value: data.transit_score != null ? `${data.transit_score}/100` : '--',
              },
            ].map(({ label, value }) => (
              <div key={label}>
...
function NeighborhoodCardSkeleton({ index }: { index: number }) {
  return (
    <div
      className="rounded-xl bg-[#0B1928] border border-white/[0.05] p-4 pl-5 animate-pulse overflow-hidden relative"
    ...
    >
      ...
      <div className="h-7 w-1/2 bg-white/[0.05] rounded mb-1" />
      <div className="h-2 w-1/3 bg-white/[0.03] rounded mb-3" />
      <div className="grid grid-cols-3 gap-1.5">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-12 bg-[#060D16]/60 rounded-lg border border-white/[0.04]" />
        ))}
      </div>
    </div>
  )
...
function EmptyState({ city, lang }: { city: string; lang: string }) {
  return (
