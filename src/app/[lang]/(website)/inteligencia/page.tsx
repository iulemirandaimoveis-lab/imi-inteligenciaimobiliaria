...
const SECTIONS = [
    ...
    },
    {
        ...
        tagColor: 'text-navy-100 bg-navy-200/10 border-navy-200/20',
    },
    {
...
export default function InteligenciaPage({ params }: { params: { lang: string } }) {
    ...
    return (
        <main className="bg-navy-950 min-h-screen">
                ...
                <div className="container-custom relative z-10">
                    <div className="max-w-4xl">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <span className="text-navy-300 font-bold uppercase tracking-[0.25em] text-[11px]">Market Intelligence</span>
                        </div>
                        <h1 className="font-display text-4xl sm:text-5xl lg:text-7xl font-bold mb-8 tracking-tight leading-tight text-white">
                            Centro de <br /><span className="text-navy-300 italic">Inteligência</span> Imobiliária
                        </h1>
                        <p className="text-white/50 text-lg sm:text-xl font-light leading-relaxed max-w-2xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {SECTIONS.map((s) => (
                            <Link
                                key={s.href}
                                href={`/${lang}/${s.href}`}
                                className="group block p-8 sm:p-10 rounded-3xl bg-navy-950 border border-white/[0.05] hover:border-navy-600/40 hover:shadow-[0_8px_32px_rgba(26,26,46,0.15)] transition-all duration-300"
                            >
                                <div className="flex items-start justify-between mb-8">
                                    <div className="w-14 h-14 bg-navy-800 text-navy-300 rounded-2xl flex items-center justify-center border border-white/[0.05] group-hover:scale-110 transition-transform">
                                ...
                                <h2 className="font-display text-2xl font-bold text-white mb-3 group-hover:text-navy-100 transition-colors">
                                    {s.label}
                                </h2>
                                <p className="text-white/50 text-sm leading-relaxed mb-6 font-light">
                                    {s.description}
                                </p>
                                <span className="flex items-center gap-2 text-navy-300 text-sm font-semibold">
                                    Acessar
                ...
                <div className="container-custom">
                    <div className="text-center mb-14">
                        <div className="flex items-center justify-center gap-3 mb-4">
                            ...
                            <span className="text-navy-300 font-bold uppercase tracking-[0.25em] text-[11px]">Nossa Metodologia</span>
                            <div className="w-8 h-px bg-navy-600" />
                        </div>
                        <h2 className="font-display text-3xl sm:text-4xl font-bold text-white">
                            Inteligência com <span className="text-navy-300 italic">Rigor Técnico</span>
                        </h2>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {PILLARS.map((p, i) => (
                            <div
                                key={i}
                                className="p-8 rounded-3xl bg-navy-950 border border-white/[0.05] text-center"
                            >
                                <div className="w-14 h-14 bg-navy-800 text-navy-300 rounded-2xl flex items-center justify-center mb-6 mx-auto border border-white/[0.05]">
                                    <p.icon className="w-6 h-6" strokeWidth={1.5} />
                                </div>
                                <h3 className="font-display text-lg font-bold text-white mb-3">{p.title}</h3>
                                <p className="text-white/50 text-sm leading-relaxed font-light">{p.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {/* CTA */}
            <section className="bg-navy-950 text-white py-20 md:py-28 text-center relative overflow-hidden border-t border-white/[0.05]">
                ...
                <div className="container-custom relative z-10">
                    <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold mb-6 text-white tracking-tight">
                        Precisa de uma <span className="text-navy-300 italic">Análise Personalizada</span>?
                    </h2>
                    ...
                    <a
                        ...
                        className="inline-flex items-center gap-3 h-14 px-10 rounded-xl bg-navy-800 text-white font-bold text-sm uppercase tracking-wider hover:bg-navy-700 transition-all shadow-[0_4px_14px_rgba(16,42,67,0.4)]"
                    >
