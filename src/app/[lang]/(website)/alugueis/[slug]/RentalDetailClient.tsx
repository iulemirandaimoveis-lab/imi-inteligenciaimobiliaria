...
export default function RentalDetailClient({ property, bookedDates, lang }: Props) {
    ...
    const whatsappMsg = encodeURIComponent(
        `Olá! Tenho interesse no imóvel "${p.name as string}" para locação. Poderia me passar mais informações?`
    )
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-6xl mx-auto px-4 mt-4">
                {photos.length > 0 ? (
                            ...
                            <div className="absolute bottom-3 left-3 flex gap-1.5">
                                {photos.slice(0, 6).map((ph, i) => (
                                    <button
                                        ...
                                        style={{ borderColor: i === photoIdx ? '#C8A44A' : 'rgba(255,255,255,0.4)' }}
                                    >
                                        <Image src={ph} alt="" width={48} height={32} className="w-full h-full object-cover" />
                                    </button>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="aspect-[16/9] md:aspect-[21/9] rounded-xl bg-gray-200 flex items-center justify-center text-gray-400">
                        <MapPin size={64} />
                    </div>
                )}
            </div>
                    ...
                    <div>
                        ...
                        {p.address && (
                            <p className="text-gray-500 mt-1 flex items-center gap-1.5">
                                <MapPin size={14} /> {p.address as string}
                            </p>
                        )}
                    </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                {amenities.map(a => (
                                    <div key={a} className="flex items-center gap-2 text-sm text-gray-700">
                                        <span className="text-[#C8A44A]">
                                            {AMENITY_ICONS[a.toLowerCase()] || <ShieldCheck size={16} />}
                                        </span>
                                        {a}
                                    </div>
                                ))}
                        ...
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-semibold min-w-[140px] text-center">
                                    {MONTH_NAMES_PT[calMonth]} {calYear}
                                </span>
                ...
                <div className="space-y-4">
                    ...
                    >
                        {(p.daily_rate as number | null) != null && (
                            <div className="mb-2">
                                <span className="text-2xl font-bold" style={{ color: '#C8A44A' }}>
