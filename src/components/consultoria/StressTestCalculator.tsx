...
export function StressTestCalculator() {
    ...
    return (
        <div className="bg-navy-950 p-8 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.2)] border border-white/[0.05] h-full flex flex-col">
            ...
            <p className="text-sm text-white/50 mb-8">Avalie a resiliência do seu investimento frente a períodos de baixa ocupação.</p>
            ...
            <div className="space-y-8 flex-grow">
                <div>
                    <div className="flex justify-between mb-4">
                        ...
                        <span className={`font-bold ${vacancyRate > 50 ? 'text-red-400' : 'text-white'}`}>{vacancyRate}%</span>
                    </div>
                    <input
                        type="range"
                        min="0"
                ...
                <div className={`p-8 rounded-2xl border-2 transition-all duration-500 flex flex-col items-center text-center ${isSafe ? 'bg-navy-950 border-white/[0.05]' : 'bg-red-500/10 border-red-500/30 shadow-lg shadow-[#EF4444]/10'}`}>
                    ...
                    <h4 className={`text-xl font-bold mb-3 font-display ${isSafe ? 'text-white' : 'text-red-400'}`}>
                        {isSafe ? 'Investimento Resiliente' : 'Alerta de Fluxo de Caixa'}
                    </h4>

                    <p className={`text-sm leading-relaxed ${isSafe ? 'text-white/50' : 'text-red-400/80'}`}>
                        {isSafe
